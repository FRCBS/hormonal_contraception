...
g1 <-
  ...
  set_node_attrs(
    node_attr = color,
    values = "#ADB6B699") %>% # the coloring if no node specified 
  set_node_attrs(
    node_attr = color,
    values = "#ED000099",
...
g2 <-
  ...
  set_node_attrs(
    node_attr = fillcolor,
    values = "#ADB6B699") %>% # color if node is not specified
  set_node_attrs(
    node_attr = fillcolor,
    values = "#ED000099", # this makes text white
    nodes = c(1)) %>% 
    set_node_attrs(
    node_attr = fontcolor,
...
g3 <-
  ...
  DiagrammeR::set_edge_attrs( # igraph has the same function, so you need to specify
    edge_attr = color,
    values = "#1B191999" # not specifying nodes makes all arrows the chosen color
  ) %>%
  DiagrammeR::set_edge_attrs( # igraph has the same function, so you need to specify
    edge_attr = color,
    values = "#42B540FF", 
...
#how to define the latent variables to get the corract adjustement set
```
No way to block backdoor paths. The issue seems to be inflammation as well as the thrombosis/embolism pathways, so we need to adjust for inflammatory conditions (as inflammation itself is latent) and thrombosis+embolism. 
...
g2 <-
  ...
  set_node_attrs(
    node_attr = fillcolor,
    values = "#AD002A99", # this makes text white 
    nodes = c(1)) %>%  # outcome
    set_node_attrs(
    node_attr = fontcolor,
    values = "black") %>% 
  ...
  set_node_attrs(
    node_attr = fillcolor,
    values = "#ED000099", 
    nodes = c(4,6,7,16,19,21)) %>%   # unobserved (unobserved variables can't be captured due to practical limitations) 
    set_node_attrs(
    node_attr = fontcolor,
    values = "black") 
...
## Add minimum adjustment set as squares
